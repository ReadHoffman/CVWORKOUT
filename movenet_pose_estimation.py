...
def update_keypoints_xy(pose,img_width,img_height):
    for i in range(len(pose)):
            x = int(pose[i][1] * img_width)
            y = int(pose[i][0] * img_height)
            print(pose)
            np.append(pose[i],[[x,y]])
            print(pose)
    return pose
...
MODEL_FILE = 'models/movenet_single_pose_thunder_ptq_edgetpu.tflite'  #slower more accurate
